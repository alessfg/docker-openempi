...
CREATE OR REPLACE FUNCTION public.removepersonvoided(OUT rowsaffected integer)
AS
$$
DECLARE
  admin_id INTEGER;
...
BEGIN
  ...
  FOR personrow IN
    SELECT * FROM person WHERE date_voided is not null
  LOOP
...
ALTER FUNCTION public.removepersonvoided(OUT rowsaffected integer) OWNER TO openempi;
