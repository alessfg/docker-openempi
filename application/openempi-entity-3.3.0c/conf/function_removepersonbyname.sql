...
CREATE OR REPLACE FUNCTION public.removepersonbyname(IN firstname varchar, IN lastname varchar, OUT rowsaffected integer)
AS
$$
DECLARE
  admin_id INTEGER;
...
BEGIN
  ...
  FOR personrow IN
    SELECT * FROM person WHERE given_name like firstname AND family_name like lastname
  LOOP
    ...
    DELETE FROM person_link_review WHERE rh_person_id = personid OR lh_person_id = personid;
...
ALTER FUNCTION public.removepersonbyname(OUT rowsaffected integer) OWNER TO openempi;
